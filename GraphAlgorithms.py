...
def distance_cities(x1, y1, x2, y2):
...
class Node:
    ...
    def add_edge(self, start, end):
        self.edges.append(Edge(start.nr, end.nr, distance_cities(start.x, start.y, end.x, end.y)))
...
class Graph:
    ...
    def does_node_exist(self, x, y):
        for node in self.list:
            if x == node.x and y == node.y:
                return True
        return False

    def is_closed_tour(self):
        for node in self.list:
            if len(node.edges) < 2:
                return False
        return True
    ...
    def add_node(self):
        while True:
            x = randint(-100, 100)
            y = randint(-100, 100)
            if not self.does_node_exist(x, y):
    ...
    def find_route_length(self, route):
        ...
        for i in range(len(route) - 1):
            ...
            distance += distance_cities(node_1.x, node_1.y, node_2.x, node_2.y)
        return distance

    def del_random_edge(self):
    ...
    def print_nodes(self):
        if self.size == 0:
            ...
            for i in range(self.size):
                print(f"City Number: {i}, [{self.list[i].x}, {self.list[i].y}]")
                for edge in self.list[i].edges:
                    print(
                        f"Route: [{edge.start}, {edge.end}], distance:  {round(edge.distance, 2)}")
                print("\n")
    ...
    def bidirectional_search(self, start, end):
        ...
        print(
            ...
            f"\nIts length is {self.find_route_length(final_route): .2f}")
